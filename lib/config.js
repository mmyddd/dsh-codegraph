 ...
 * - vendored 运行时内部的 `auto_provision`、`session_start_cooldown_ms` 等仍由
 *   OMO 配置（`~/.omo/omo.jsonc` 或项目 `.omo/omo.jsonc`）或默认值决定，插件配置不重复实现。
 */
export const Config = z.object({
  ...
  /** 受管安装目录（原 `CODEGRAPH_INSTALL_DIR`）。留空默认 `~/.omo/codegraph`。 */
