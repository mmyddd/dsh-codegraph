 ...
 * 与原 lazycodex hooks matcher `^(codegraph[._].*|mcp__codegraph__.*|mcp__plugin_omo_codegraph__.*)$` 对齐，
 * 额外兼容 `mcp__<serverName>__codegraph_*`。
 */
export function isCodegraphToolName(toolName, serverName = 'codegraph') {
  ...
  if (toolName.startsWith('mcp__')) {
    // 主命名空间 + lazycodex 的旧命名空间 mcp__plugin_omo_codegraph__*
    return toolName.includes(`mcp__${serverName}__`) || toolName.startsWith('mcp__plugin_omo_codegraph__')
  }
  return false
}
