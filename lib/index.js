    ...
    ctx.on('agent/session-start', ({ agent }) => {
      try {
        ...
        if (result.action === 'scheduled') {
          ...
          // 对齐 lazycodex SessionStart hook：把「已调度后台引导」通知注入上下文。
          try {
            agent.inject(
              createUserMessage({
                content: [{ type: 'text', text: 'dsh-codegraph: CodeGraph bootstrap scheduled in background.' }],
                source: { kind: 'plugin', plugin: 'dsh-codegraph' },
              }),
            )
          } catch (injectError) {
            ctx.logger.warn(`dsh-codegraph: failed to inject bootstrap notice: ${String(injectError)}`)
          }
        }
      } catch (error) {
